...
public class VehicleControl : MonoBehaviourPunCallbacks
{
    ...
    void OnTriggerEnter(Collider other)
    {
        if (view.IsMine)
        {
            if(other.gameObject.tag == "LapCounter1")
            {
                lapTextScore++;
                lapText.text = lapTextScore.ToString();
                Destroy(other.gameObject);
            }
            if (other.gameObject.tag == "LapCounter2")
            {
                lapTextScore++;
                lapText.text = lapTextScore.ToString();
                Destroy(other.gameObject);
            }
            if (other.gameObject.tag == "LapCounter3")
            {
                lapTextScore++;
                lapText.text = lapTextScore.ToString();
                Destroy(other.gameObject);
            }
            if (other.gameObject.tag == "Finished" && lapTextScore == 3)
            {
                PhotonView phView = other.gameObject.GetComponent<PhotonView>();
                phView.RPC("RacePosition", RpcTarget.AllViaServer, PhotonNetwork.NickName);
                phView.RPC("EnableLeaderboardBackground", PhotonNetwork.LocalPlayer);
                lapTextScore++;
                lapText.text = "Race Over!";
                activeControl = false;
            }
            /*if (other.gameObject.name == "abcd")
            {
                PhotonView phView = other.gameObject.GetComponent<PhotonView>();
                phView.RPC("RacePosition", RpcTarget.AllViaServer, PhotonNetwork.NickName);
                phView.RPC("EnableLeaderboardBackground", PhotonNetwork.LocalPlayer);
    ...
    private void OnTriggerExit(Collider other)
    {
        if (view.IsMine)
        {
            if (other.gameObject.name == "EnableCarInputs")
            {
                activeControl = true;
            }
        }
    ...
    void FixedUpdate()
    {
        ...
        : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
                ...
                if (shift && (currentGear > 1 && speed > 50.0f) && shifmotor && Mathf.Abs(steer) < 0.2f)
                {
                    ...
                    carSounds.nitro.volume = Mathf.Lerp(carSounds.nitro.volume, 0.4f, Time.deltaTime * 10.0f);
            ...
            if (Pitch == 1)
            {
                carSounds.IdleEngine.volume = Mathf.Lerp(carSounds.IdleEngine.volume, 0.4f, 0.1f);
                carSounds.LowEngine.volume = Mathf.Lerp(carSounds.LowEngine.volume, 0.4f, 0.1f);
            ...
            else
            {
                ...
                carSounds.IdleEngine.volume = Mathf.Lerp(carSounds.IdleEngine.volume, 1.2f - Pitch, 0.1f);


                if ((Pitch > PitchDelay || accel > 0) && shiftTime == 0.0f)
                {
                    carSounds.LowEngine.volume = Mathf.Lerp(carSounds.LowEngine.volume, 0.0f, 0.2f);
                    carSounds.HighEngine.volume = Mathf.Lerp(carSounds.HighEngine.volume, 0.4f, 0.1f);
                }
                else
                {
                ...
                carSounds.HighEngine.pitch = Pitch/3;
                carSounds.LowEngine.pitch = Pitch/3;
